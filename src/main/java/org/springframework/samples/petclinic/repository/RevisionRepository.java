...
@Query("SELECT DISTINCT revision FROM Revision revision left join fetch recision.descripcion WHERE revision.fechaRevision:=fechaRevision")
public Collection<Revision> findByFechaRevision(@Param("fechaRevision") LocalDate fechaRevision);

void delete (Revision revision) throws DataAccessException;
}
